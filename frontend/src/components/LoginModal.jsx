...
export default function LoginModal({isOpen ,setIsOpen }) {
    ...
    const handleLogin = () => {
        ...
        res.then(json => {
            ...
            if(json.success) {
                if(json.user.user_type === 'buyer') {
                    navigate(`/${json.user.user_id}`);
                }
                else {
                    navigate(`/seller/${json.user.user_id}`)
                }
            }
        });
        closeModal();
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-2 flex flex-col justify-around items-start">
                    ...
                    />
                    <input type="password" name="" id=""  placeholder = "Password" onChange = {(event) => setPassword(event.target.value)}
                        className = 'w-full border-b-2 border-black p-2 my-2 outline-none font-semibold '
                    />
                  </div>
